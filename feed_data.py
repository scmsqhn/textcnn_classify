...
tf.flags.DEFINE_string("checkpoint_dir", "/data/ner-bilstm-dnn/cnn-text-classification-tf/runs/1530688756/checkpoints", "Checkpoint directory from training run")
                ...
                if cont == -1:
                    ...
                    for i in range(32):
                        ...
                        if i< len(cont):
                            ...
                            sent = cont[-1]
                        batch.append(dh.sentFromDct(sent))
                batch_predictions = sess.run(predictions, {input_x: batch, dropout_keep_prob: 1.0})
                outq.put(batch_predictions[:len(cont)])
                print("btch_predictions")
